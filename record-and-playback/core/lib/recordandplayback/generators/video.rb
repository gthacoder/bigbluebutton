...
module BigBlueButton
  ...
  def BigBlueButton.process_sign_language_videos(target_dir, temp_dir, meeting_id, output_width, output_height)
    ...
    # check for no webcam found and exit method
    return if start_evt.empty?
    ...
    # create webm video and mux audio
    command = "ffmpeg -i #{target_dir}/audio.ogg -i #{concat_vid} -loglevel fatal -v -10 -vcodec libvpx -b 1000k -threads 0  -map 1:0 -map 0:0 -ar 22050 #{target_dir}/signlanguage.webm"
    BigBlueButton.execute(command)
  end
end
