...
if ($playback == "presentation")
  ...
  BigBlueButton.logger.info("RUNNING SLIDES_NEW.RB - Publishing #{$meeting_id}")
	...
	if not FileTest.directory?(target_dir)
		...
		if File.exist?("#{$process_dir}/signlanguage.webm")
  		  BigBlueButton.logger.info("Making video dir")
  		  video_dir = "#{package_dir}/video"
		  FileUtils.mkdir_p video_dir
		  BigBlueButton.logger.info("Made video dir - copying: #{$process_dir}/signlanguage.webm to -> #{video_dir}")
		  FileUtils.cp("#{$process_dir}/signlanguage.webm", video_dir)
		  BigBlueButton.logger.info("Copied .webm file")
		end
